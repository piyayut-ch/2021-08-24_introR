...
# Miniconda
## install miniconda
- download [miniconda3](https://docs.conda.io/en/latest/miniconda.html) เลือกระหว่าง `Miniconda3 Windows 64-bit` หรือ `Miniconda3 Windows 32-bit` ขึ้นอยู่กับ windows system
- เปิดไฟล์ installer ที่ download ไว้
- คลิ้ก Next
- คลิ้ก I Agree
- เลือก Just Me และ คลิ้ก Next
- เปลี่ยน Desitnation Folder เป็น `C:\Users\[USERNAME]\Miniconda3` โดยที่ USERNAME หมายถึง ชื่อตอน log-in เข้าเครื่อง
- คลิ้ก Next ไปเรื่อย ๆ จนดิดตั้งเสร็จ 

## set environment path
- คลิ้ก ขวา ที่ My Computer แล้วเลือก Properties
- เลือก Advance system setting &rarr; Advanced &rarr; Environment Variables
- เลือก Variable ชื่อ path แล้วเลือก Edit
- คลิ้ก New แล้วเพิ่ม path ดังนี้  
...
- คลิ้กขวา ที่ icon บน taskbar เลือก `pin to taskbar`
- พิมพ์คำสั่ง `pip install jupyterlab`
- พิมพ์คำสั่ง `conda install -c conda-forge nodejs`
...
- คลิ้ก Next
- คลิ้ก I Agree
- เลือก Just Me และ คลิ้ก Next ไปเรื่อย ๆ จนดิดตั้งเสร็จ

## set environment path
- คลิ้ก ขวา ที่ My Computer แล้วเลือก Properties
- เลือก Advance system setting &rarr; Advanced &rarr; Environment Variables
- เลือก Variable ชื่อ path แล้วเลือก Edit
- คลิ้ก New แล้วเพิ่ม path `C:\Program Files\R\[R-x.x.x]\bin\x64` โดยที่ x64 หมายถึง version 64 bit  

## install IRkernel  
- เปิด Anaconda Powershell Prompt ที่ taskbar ซึ่งเราได้ pin ไว้แล้ว
- พิมพ์คำสั่ง `R.exe` เพื่อให้ run program `R`
- พิมพ์คำสั่ง `install.packages('IRkernel')` เพื่อติดตั้ง library
- พิมพคำสั่ง `IRkernel::installspec()` เพื่อเพิ่ม kernel R ไปใน jupyter lab
- พิมพคำสั่ง `q()` และตามด้วย `n` เพื่อออกจากโปรแกรม R 


# Test
- เปิด Anaconda Powershell Prompt
- พิมพ์คำสั่ง `jupyter lab` defaullt browser จะเปิดขึ้น `http://localhost:8888/lab`
- สร้าง folder ใหม่ ใน Document เช่น `training` เป็นต้น
